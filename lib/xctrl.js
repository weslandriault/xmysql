...
class xctrl {
  ...
  async bulkDelete(req, res) {
    ...
    let whereQuery = '';
    const definedWhereQuery = req.query?._where;
    ...
    if (req.query?._ids) {
      const primaryKeyName = this.mysql.getPrimaryKeyName(req.app.locals._tableName);
      whereQuery = `(${primaryKeyName},in,${req.query._ids})`;
    }

    if (req.query?._where) {
      whereQuery += whereQuery ?
        `~and${req.query?._where}` :
        req.query?._where;
    }
